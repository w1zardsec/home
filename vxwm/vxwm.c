...
#if !BAR_PADDING
#if BAR_WIDTH
				XMoveResizeWindow(dpy, m->barwin, m->wx + (m->ww - m->ww * bar_width) / 2, m->by, m->ww * bar_width, bh);
#else
				XMoveResizeWindow(dpy, m->barwin, m->wx, m->by, m->ww, bh);
#endif
#else
#if BAR_WIDTH
        XMoveResizeWindow(dpy, m->barwin, m->wx + (m->ww - m->ww * bar_width) / 2 + sp, m->by + vp, m->ww * bar_width - 2 * sp, bh);
#else
...
#else
//  drw_text(drw, barw - sw - 2 * sp, 0, sw, bh, 0, stext, 0); this was causing the solved issue 2
    drw_text(drw, barw - tw - 2 * sp, 0, tw, bh, 0, stext, 0);
#endif
	}
...
#if !BAR_PADDING
#if BAR_WIDTH
	XMoveResizeWindow(dpy, selmon->barwin, selmon->wx + (selmon->ww - selmon->ww * bar_width) / 2, selmon->by, selmon->ww * bar_width, bh);
#else
	XMoveResizeWindow(dpy, selmon->barwin, selmon->wx, selmon->by, selmon->ww, bh);
#endif
#else
#if BAR_WIDTH
  XMoveResizeWindow(dpy, selmon->barwin, selmon->wx + (selmon->ww - selmon->ww * bar_width) / 2 + sp, selmon->by + vp, selmon->ww * bar_width - 2 * sp, bh);
#else
...
#if !BAR_PADDING
#if BAR_WIDTH
		m->barwin = XCreateWindow(dpy, root, m->wx + (m->ww - m->ww * bar_width) / 2, m->by, m->ww * bar_width, bh, 0, DefaultDepth(dpy, screen),
#else
		m->barwin = XCreateWindow(dpy, root, m->wx, m->by, m->ww, bh, 0, DefaultDepth(dpy, screen),
#endif
#else
#if BAR_WIDTH
    m->barwin = XCreateWindow(dpy, root, m->wx + (m->ww - m->ww * bar_width) / 2 + sp, m->by + vp, m->ww * bar_width - 2 * sp, bh, 0, DefaultDepth(dpy, screen),
#else
